...
exports.setupTeamSocketEvents = setupTeamSocketEvents;
const crypto_1 = __importDefault(require("crypto"));
const teamTrucoGame_1 = require("../game/teamTrucoGame");
...
const pendingTeamCreations = new Set();
const pendingTeamJoins = new Set();
const otherTeam = (team) => team === 'A' ? 'B' : 'A';
const scoreFor = (room, team) => team === 'A' ? room.scoreA : room.scoreB;
function addScore(room, team, points) {
    if (team === 'A')
        room.scoreA += points;
    else
        room.scoreB += points;
...
function roomLog(room, text) {
    ...
    if (room.logs.length > 200)
        room.logs.splice(0, room.logs.length - 200);
}
function pushTeamChat(room, team, fromSeat, fromUserId, text) {
...
function seatOrderFrom(manoSeat) {
    return [0, 1, 2, 3].map(i => (manoSeat + i) % 4);
}
function getSeat(room, seat) {
...
function releaseTeamRoomPresence(room) {
    for (const player of room.players) {
        (0, activeGameRegistry_1.releaseActiveGame)(player.userId, '2v2', room.roomId);
    }
}
function makeRoomId() {
    return `2v2_${crypto_1.default.randomBytes(4).toString('hex')}`;
}
function makeEntryToken() {
    return crypto_1.default.randomBytes(6).toString('hex');
}
function getRoundCards(room, seat) {
    return room.round?.getAllCardsForSeat(seat) || [];
}
function seatHasFlor(room, seat) {
    return !!room.round && (0, trucoEngine_1.hasFlor)(getRoundCards(room, seat));
}
function teamHasFlor(room, team) {
    return room.players.some(p => p.team === team && seatHasFlor(room, p.seat));
}
function envidoPoints(room) {
    const chain = room.envidoChain;
    if (!chain.length)
        return { accepted: 0, declined: 1 };
    const value = (call) => call === 'ENVIDO' ? 2 : call === 'REAL_ENVIDO' ? 3 : 0;
...
function bestEnvidoForTeam(room, team) {
    ...
    const candidates = room.players
        ...
        .map(p => ({ seat: p.seat, userId: p.userId, score: (0, trucoEngine_1.calculateEnvido)(getRoundCards(room, p.seat)) }));
    candidates.sort((a, b) => b.score - a.score || order.indexOf(a.seat) - order.indexOf(b.seat));
    return candidates[0];
}
function bestFlorForTeam(room, team) {
    const order = seatOrderFrom(room.manoSeat);
    const candidates = room.players
        .filter(p => p.team === team && seatHasFlor(room, p.seat))
        .map(p => ({ seat: p.seat, userId: p.userId, score: (0, trucoEngine_1.calculateFlor)(getRoundCards(room, p.seat)) }));
    candidates.sort((a, b) => b.score - a.score || order.indexOf(a.seat) - order.indexOf(b.seat));
    return candidates[0] || null;
}
function chooseByMano(room, a, b) {
    if (a.score > b.score)
        return (0, teamTrucoGame_1.teamForSeat)(a.seat);
    if (b.score > a.score)
        return (0, teamTrucoGame_1.teamForSeat)(b.seat);
    const order = seatOrderFrom(room.manoSeat);
    return order.indexOf(a.seat) < order.indexOf(b.seat) ? (0, teamTrucoGame_1.teamForSeat)(a.seat) : (0, teamTrucoGame_1.teamForSeat)(b.seat);
}
function canRaiseEnvido(room, callType) {
    const chain = room.envidoChain;
    if (chain.includes('FALTA_ENVIDO'))
        return false;
    if (callType === 'FALTA_ENVIDO')
        return true;
    if (callType === 'REAL_ENVIDO')
        return !chain.includes('REAL_ENVIDO');
    if (callType === 'ENVIDO') {
        return !chain.includes('REAL_ENVIDO') && chain.filter(c => c === 'ENVIDO').length < 2;
    }
    return false;
}
function canInitiateTantos(room, seat) {
    if (!room.round || room.round.currentTrickIndex !== 0)
        return false;
    if (room.round.envidoResolved || room.round.florResolved)
        return false;
...
function opponentStillHadEnvidoOpportunity(room, foldingTeam) {
    if (!room.round || room.round.currentTrickIndex !== 0)
        return false;
    if (room.round.envidoResolved || room.round.florResolved)
        return false;
    ...
    return room.players.some(player => {
        if (player.team !== winningTeam)
            return false;
        if (room.round.hasSeatPlayedAnyCard(player.seat))
            return false;
        // Con Flor habilitada, un jugador con Flor no tiene Envido disponible.
        if (room.withFlor && seatHasFlor(room, player.seat))
            return false;
        return true;
    });
}
function disconnectedPlayers(room) {
    return room.players.filter(player => !player.connected);
}
function getTeamTimerContext(room) {
    if (room.phase !== 'PLAYING' || !room.round || room.transitioning || room.round.isFinished)
        return null;
    if (room.envidoDeclarationActive) {
        ...
        return {
            ...
            seat,
            team: player?.team ?? (0, teamTrucoGame_1.teamForSeat)(seat),
...
function teamDisconnectSnapshot(room) {
    const now = Date.now();
    return room.players
...
function emitTeamTimer(io, room) {
    const context = getTeamTimerContext(room);
    if (!context || !room.turnDeadline)
        return;
...
function startTeamTurnTimer(io, room, milliseconds = 30000) {
    const context = getTeamTimerContext(room);
    if (!context || disconnectedPlayers(room).length)
        return;
    stopTeamTurnInterval(room);
...
function syncTeamTurnTimer(io, room) {
    const context = getTeamTimerContext(room);
    if (!context) {
        clearTeamTurnTimer(room);
        return;
    }
    if (disconnectedPlayers(room).length)
        return;
...
function resumeTeamTurnTimer(io, room) {
    if (disconnectedPlayers(room).length)
        return;
    const context = getTeamTimerContext(room);
    if (!context) {
...
function clearTeamDisconnectTimer(room) {
...
function finishTeamMatchByForfeit(io, room, loser, reason) {
...
function ensureTeamDisconnectInterval(io, room) {
    if (room.disconnectInterval)
        return;
    room.disconnectInterval = setInterval(() => {
        ...
        if (loser)
            finishTeamMatchByForfeit(io, room, loser, 'DISCONNECT_TIMEOUT');
    }, 1000);
}
function startTeamDisconnectGrace(io, room, seat) {
    if (room.phase !== 'PLAYING')
        return;
...
function restoreTeamSeatConnection(io, socket, room, seat) {
    const wasDisconnected = !seat.connected || !!room.disconnectDeadlines[seat.seat];
    seat.socketId = socket.id;
    seat.connected = true;
    socket.join(room.roomId);
    if (room.disconnectDeadlines[seat.seat])
        delete room.disconnectDeadlines[seat.seat];
    if (wasDisconnected)
        roomLog(room, `🔌 ${seat.userId} se reconectó.`);
    ...
    if (!remainingDisconnected.length) {
        ...
        room.disconnectDeadlines = {};
        if (room.phase === 'PLAYING')
            resumeTeamTurnTimer(io, room);
    }
    else {
        io.to(room.roomId).emit('team_disconnect_tick', { roomId: room.roomId, players: teamDisconnectSnapshot(room) });
    }
}
function handleTeamTimeout(io, room, expiredContext) {
    if (room.phase !== 'PLAYING' || !room.round || disconnectedPlayers(room).length)
        return;
    const context = getTeamTimerContext(room);
    ...
    if (room.envidoDeclarationActive) {
        ...
        if (!seat)
            return;
        const score = (0, trucoEngine_1.calculateEnvido)(getRoundCards(room, seat.seat));
...
function availableActions(room, seat) {
    if (disconnectedPlayers(room).length)
        return [];
    if (room.phase !== 'PLAYING' || !room.round || room.transitioning || room.round.isFinished)
        return [];
    ...
    if (room.envidoDeclarationActive) {
        if (room.envidoSonBuenasSeat != null) {
            if (seat.seat === room.envidoSonBuenasSeat)
                actions.push('SON_BUENAS_ENVIDO');
            return actions;
        }
        const currentSeat = room.envidoDeclarationOrder[room.envidoDeclarationIndex];
        if (seat.seat !== currentSeat)
            return actions;
        const myScore = (0, trucoEngine_1.calculateEnvido)(getRoundCards(room, seat.seat));
        const best = room.envidoWinningScore;
        if (best == null || myScore > best)
            actions.push('CANTAR_ENVIDO');
        else
            actions.push('SON_BUENAS_ENVIDO');
        return actions;
    }
    const pending = room.pendingCall;
    if (pending) {
        if (seat.team !== pending.responderTeam)
            return actions;
        if (pending.kind === 'TRUCO') {
            actions.push('QUIERO_TRUCO', 'NO_QUIERO_TRUCO');
            if (pending.proposedStake === 2)
                actions.push('RETRUCO');
            if (pending.proposedStake === 3)
                actions.push('VALE_4');
        ...
        if (pending.kind === 'ENVIDO') {
            ...
            for (const call of ['ENVIDO', 'REAL_ENVIDO', 'FALTA_ENVIDO']) {
                if (canRaiseEnvido(room, call))
                    actions.push(call);
            }
            if (room.withFlor && canInitiateTantos(room, seat) && seatHasFlor(room, seat.seat))
                actions.push('FLOR');
    ...
    if (room.trucoStake === 1 && room.trucoLastRaiserTeam == null)
        actions.push('TRUCO');
    else if (room.trucoStake === 2 && room.trucoLastRaiserTeam !== seat.team)
        actions.push('RETRUCO');
    else if (room.trucoStake === 3 && room.trucoLastRaiserTeam !== seat.team)
        actions.push('VALE_4');
    return actions;
}
...
function buildState(room, mySeat) {
    ...
    return {
        ...
        payout: (0, userService_1.calculateTeamMatchPayout)(room.betAmount),
        ...
        round: round ? {
            ...
            myEnvidoScore: (0, trucoEngine_1.calculateEnvido)(getRoundCards(room, mySeat.seat)),
...
function scheduleReturnAllToTeamLobby(io, room) {
    if (room.finishLobbyTimer)
        return;
    room.finishLobbyTimer = setTimeout(() => {
        room.finishLobbyTimer = undefined;
        if (teamRooms.get(room.roomId) !== room || room.phase !== 'MATCH_OVER' || !room.settlementComplete)
            return;
...
async function settleTeamFinancials(io, room, winnerTeam, reason) {
    if (room.settlementComplete || room.settlementInProgress)
        return;
    const payout = (0, userService_1.calculateTeamMatchPayout)(room.betAmount);
    if (room.betAmount <= 0) {
    ...
    const result = await (0, userService_1.settleTeamMatchOnce)({
        roomId: room.roomId,
        winnerTeam,
...
function broadcastTables(io) {
    io.emit('team_update_tables', availableRooms());
}
function resetBids(room) {
    room.pendingCall = null;
    room.suspendedTruco = null;
    room.trucoStake = 1;
...
function startRound(io, room) {
    ...
    room.round = new teamTrucoGame_1.TeamTrucoRound(ids, room.manoSeat, room.targetPoints, room.withFlor);
...
function finishMatchOrScheduleNext(io, room, winnerTeam, points, reason) {
    if (!room.round || room.transitioning)
        return;
    room.round.isFinished = true;
    room.round.winnerTeam = winnerTeam;
...
function resumeSuspendedTruco(room) {
...
function emitRoomAudio(io, room, file) {
    io.to(room.roomId).emit('team_audio', { file });
}
function emitCallAudio(io, room, callType) {
    const audioByCall = {
        TRUCO: 'truco.mp3',
        RETRUCO: 'retruco.mp3',
        VALE_4: 'vale 4.mp3',
    ...
    if (file)
        emitRoomAudio(io, room, file);
}
function startEnvidoDeclarations(io, room) {
    if (!room.round || !room.pendingCall || room.pendingCall.kind !== 'ENVIDO')
        return;
...
function finishEnvidoDeclarations(io, room) {
    if (!room.round || room.envidoWinningSeat == null || room.envidoWinningScore == null)
        return;
...
function handleEnvidoDeclaration(io, socket, room, seat, callType) {
    if (!room.round || !room.envidoDeclarationActive)
        return;
    const currentSeat = room.envidoDeclarationOrder[room.envidoDeclarationIndex];
    if (seat.seat !== currentSeat) {
    ...
    if (score >= 1 && score <= 38)
        emitRoomAudio(io, room, `${score}.mp3`);
    if (room.envidoDeclarationIndex === 0) {
        room.envidoDeclarationIndex = 1;
...
function resolveEnvido(io, room, accepted) {
    if (!room.round || !room.pendingCall || room.pendingCall.kind !== 'ENVIDO')
        return;
    const pending = room.pendingCall;
    const points = envidoPoints(room);
...
function resolveFlor(io, room, accepted) {
    if (!room.round || !room.pendingCall || room.pendingCall.kind !== 'FLOR')
        return;
    const pending = room.pendingCall;
...
function startFlorCall(io, room, seat) {
    if (!room.round || !seatHasFlor(room, seat.seat))
        return;
    ...
    if (!teamHasFlor(room, otherTeam(seat.team))) {
        ...
        if (room.phase === 'MATCH_OVER')
            void settleTeamFinancials(io, room, seat.team, 'SCORE_FLOR');
        return;
    }
    room.pendingCall = {
...
function startEnvidoCall(io, room, seat, callType) {
    if (!room.round)
        return;
    if (room.pendingCall?.kind === 'TRUCO') {
        room.suspendedTruco = room.pendingCall;
...
function startTrucoCall(io, room, seat, callType) {
...
function handleCall(io, socket, room, seat, callType) {
    if (!room.round)
        return;
    ...
    if (callType === 'ME_VOY_AL_MAZO') {
        ...
        return finishMatchOrScheduleNext(io, room, otherTeam(seat.team), foldPoints, `${seat.userId} se fue al mazo${uncalledEnvidoPoint ? ' con Envido todavía disponible para el equipo rival' : ''}`);
    }
    ...
    if (callType === 'TRUCO' || callType === 'RETRUCO' || callType === 'VALE_4') {
        return startTrucoCall(io, room, seat, callType);
    }
    ...
    if (callType === 'ENVIDO' || callType === 'REAL_ENVIDO' || callType === 'FALTA_ENVIDO') {
        if (room.pendingCall?.kind === 'ENVIDO') {
            room.envidoChain.push(callType);
            room.pendingCall = {
                kind: 'ENVIDO', callType: callType, callerSeat: seat.seat,
                callerTeam: seat.team, responderTeam: otherTeam(seat.team)
            };
        ...
        return startEnvidoCall(io, room, seat, callType);
    }
    if (callType === 'FLOR')
    ...
    if ((callType === 'CONTRAFLOR' || callType === 'CONTRAFLOR_AL_JUEGO') && room.pendingCall?.kind === 'FLOR') {
        ...
        room.pendingCall = {
            kind: 'FLOR', callType: callType, callerSeat: seat.seat,
            callerTeam: seat.team, responderTeam: otherTeam(seat.team)
        };
        roomLog(room, `🌸 ${seat.userId} cantó ${formatCall(callType)}.`);
        emitCallAudio(io, room, callType);
...
function setupTeamSocketEvents(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('team_create_room', async ({ userId, targetPoints, withFlor, betAmount }) => {
            ...
            if (!cleanUser)
                return socket.emit('team_error', { message: 'Ingresá un usuario para crear la mesa.' });
            try {
            ...
            if (pendingTeamCreations.has(userKey))
                return socket.emit('team_error', { message: 'La mesa ya se está creando. Esperá un instante.' });
            const already = Array.from(teamRooms.values()).find(r => r.phase !== 'MATCH_OVER' && getSeatByUser(r, cleanUser));
            if (already)
                return socket.emit('team_error', { message: `Ya estás en la mesa ${already.roomId}.` });
            ...
            try {
                if (!(0, activeGameRegistry_1.claimActiveGame)(cleanUser, '2v2', roomId)) {
                    return socket.emit('team_error', { message: 'Ya estás participando en otra mesa.' });
                }
                claimed = true;
                const debit = await (0, userService_1.debitTeamRoomEntry)(roomId, cleanUser, bet, entryToken);
                if (!debit.success) {
                    (0, activeGameRegistry_1.releaseActiveGame)(cleanUser, '2v2', roomId);
                    claimed = false;
                ...
                if (debitSucceeded && !roomStored) {
                    try {
                        await (0, userService_1.refundTeamRoomEntry)(roomId, cleanUser, bet, entryToken);
                    }
                    catch { }
                }
                if (claimed && !roomStored)
                    (0, activeGameRegistry_1.releaseActiveGame)(cleanUser, '2v2', roomId);
        ...
        socket.on('team_join_room', async ({ roomId, userId }) => {
            ...
            if (!room || room.phase !== 'WAITING')
                return socket.emit('team_error', { message: 'La mesa ya no está disponible.' });
            if (!cleanUser)
                return socket.emit('team_error', { message: 'Usuario inválido.' });
            try {
                if (!(await (0, userService_1.userExistsFresh)(cleanUser))) {
                    socket.emit('team_session_invalid', { message: 'Tu cuenta ya no existe. Volvé a iniciar sesión.' });
                    return;
            ...
            if (room.players.length >= 4)
                return socket.emit('team_error', { message: 'La mesa ya está completa.' });
            if (getSeatByUser(room, cleanUser))
                return socket.emit('team_error', { message: 'Ese usuario ya está sentado en la mesa.' });
            const elsewhere = Array.from(teamRooms.values()).find(r => r.roomId !== room.roomId && r.phase !== 'MATCH_OVER' && getSeatByUser(r, cleanUser));
            if (elsewhere)
                return socket.emit('team_error', { message: `Ese usuario ya está en ${elsewhere.roomId}.` });
            const activePresence = (0, activeGameRegistry_1.getActiveGamePresence)(cleanUser);
            if (activePresence) {
                return socket.emit('team_error', {
                    message: activePresence.mode === '1v1'
            ...
            if (pendingTeamJoins.has(joinKey))
                return socket.emit('team_error', { message: 'Ya se está procesando tu ingreso a esta mesa.' });
            ...
            let seated = false;
            try {
                if (!(0, activeGameRegistry_1.claimActiveGame)(cleanUser, '2v2', room.roomId)) {
                    return socket.emit('team_error', { message: 'Ya estás participando en otra mesa.' });
                }
                claimed = true;
                const debit = await (0, userService_1.debitTeamRoomEntry)(room.roomId, cleanUser, room.betAmount, entryToken);
                if (!debit.success) {
                    (0, activeGameRegistry_1.releaseActiveGame)(cleanUser, '2v2', room.roomId);
                    claimed = false;
                ...
                if (liveRoom !== room || room.phase !== 'WAITING' || room.players.length >= 4 || getSeatByUser(room, cleanUser)) {
                    await (0, userService_1.refundTeamRoomEntry)(room.roomId, cleanUser, room.betAmount, entryToken);
                    (0, activeGameRegistry_1.releaseActiveGame)(cleanUser, '2v2', room.roomId);
                    claimed = false;
                ...
                const player = {
                    ...
                    socketId: socket.id,
                    connected: true,
                    team: (0, teamTrucoGame_1.teamForSeat)(seatNumber),
                    entryToken
                };
        ...
        socket.on('team_leave_waiting_room', async ({ roomId }) => {
            ...
            if (!seat)
                return;
            const refund = await (0, userService_1.refundTeamRoomEntry)(room.roomId, seat.userId, room.betAmount, seat.entryToken);
            if (!refund.success) {
            ...
            (0, activeGameRegistry_1.releaseActiveGame)(seat.userId, '2v2', room.roomId);
            room.players.forEach((p, index) => {
                p.seat = index;
                p.team = (0, teamTrucoGame_1.teamForSeat)(index);
            });
        ...
        socket.on('team_check_active_room', async ({ userId }) => {
            ...
            if (room)
                socket.emit('team_active_room_found', { roomId: room.roomId });
        });
        socket.on('team_reconnect_room', async ({ roomId, userId }) => {
            const room = teamRooms.get(String(roomId || ''));
            if (!room)
                return socket.emit('team_reconnect_failed');
            const reconnectUser = String(userId || '').trim();
            try {
                if (!(await (0, userService_1.userExistsFresh)(reconnectUser))) {
                    socket.emit('team_session_invalid', { message: 'Tu cuenta ya no existe. Volvé a iniciar sesión.' });
                    return;
                }
            }
            ...
            if (room.phase === 'MATCH_OVER' && room.betAmount > 0 && !room.settlementComplete) {
                const winnerTeam = room.matchWinnerTeam || (room.scoreA >= room.targetPoints ? 'A' : 'B');
        ...
        socket.on('team_request_room_state', async ({ roomId, userId }) => {
            ...
            try {
                if (!(await (0, userService_1.userExistsFresh)(stateUser))) {
                    socket.emit('team_session_invalid', { message: 'Tu cuenta ya no existe. Volvé a iniciar sesión.' });
                    return;
                }
            }
        ...
        socket.on('team_play_card', ({ roomId, cardId }) => {
            const room = teamRooms.get(String(roomId || ''));
            if (!room || !room.round || room.phase !== 'PLAYING' || room.transitioning)
                return;
            const seat = getSeatBySocket(room, socket.id);
            if (!seat)
                return socket.emit('team_error', { message: 'No pertenecés a esta mesa.' });
            if (room.pendingCall)
                return socket.emit('team_error', { message: 'Primero hay que responder al canto pendiente.' });
            if (!availableActions(room, seat).includes('PLAY_CARD'))
                return socket.emit('team_error', { message: 'No es tu turno.' });
            const result = room.round.playCard(seat.seat, String(cardId || ''));
            if (!result.success)
                return socket.emit('team_error', { message: result.message || 'No se pudo jugar la carta.' });
        ...
        socket.on('team_send_call', ({ roomId, callType }) => {
            const room = teamRooms.get(String(roomId || ''));
            if (!room || room.phase !== 'PLAYING' || room.transitioning)
                return;
            const seat = getSeatBySocket(room, socket.id);
            if (!seat)
                return socket.emit('team_error', { message: 'No pertenecés a esta mesa.' });
        ...
        socket.on('team_restart_match', ({ roomId }) => {
            ...
            const seat = getSeatBySocket(room, socket.id);
            if (!seat || seat.seat !== 0)
                return socket.emit('team_error', { message: 'Solo J1 puede iniciar la revancha.' });
            if (room.betAmount > 0)
                return socket.emit('team_error', { message: 'Para una revancha con fichas deben crear una mesa nueva, así se debitan nuevamente las 4 entradas.' });
            const reclaimed = [];
            for (const player of room.players) {
                if (!(0, activeGameRegistry_1.claimActiveGame)(player.userId, '2v2', room.roomId)) {
                    for (const claimed of reclaimed)
                        (0, activeGameRegistry_1.releaseActiveGame)(claimed.userId, '2v2', room.roomId);
                    return socket.emit('team_error', {
        ...
        socket.on('team_show_cards_to_teammate', ({ roomId }) => {
            const room = teamRooms.get(String(roomId || ''));
            if (!room || !room.round)
                return socket.emit('team_error', { message: 'No hay una mano activa para mostrar.' });
            const seat = getSeatBySocket(room, socket.id);
            if (!seat)
                return socket.emit('team_error', { message: 'No pertenecés a esta mesa.' });
            ...
            if (!cards.length)
                return socket.emit('team_error', { message: 'Ya no tenés cartas en la mano para mostrar.' });
        ...
        socket.on('team_send_private_chat', ({ roomId, text }) => {
            ...
            if (cleanText.length > 160)
                return socket.emit('team_error', { message: 'El mensaje es demasiado largo.' });
