...
function setupSocketEvents(io) {
    ...
    function calculateEnvidoPoints(chain) {
        let accepted = 0;
        let declined = 1;
        if (chain.length === 1) {
            if (chain[0] === 'ENVIDO') {
            ...
            if (chain[0] === 'REAL_ENVIDO') {
                accepted = 3;
                declined = 1;
            }
            if (chain[0] === 'FALTA_ENVIDO') {
                accepted = 30;
                declined = 1;
            }
        }
        else if (chain.length === 2) {
            if (chain[0] === 'ENVIDO' && chain[1] === 'ENVIDO_ENVIDO') {
                accepted = 4;
                declined = 2;
            }
            if (chain[0] === 'ENVIDO' && chain[1] === 'REAL_ENVIDO') {
                accepted = 5;
                declined = 2;
            }
            if (chain[0] === 'REAL_ENVIDO' && chain[1] === 'FALTA_ENVIDO') {
                accepted = 30;
                declined = 3;
            }
        }
        else if (chain.length >= 3) {
            if (chain.includes('REAL_ENVIDO')) {
                accepted = 7;
                declined = 4;
            }
            else {
    ...
    function resolveTrucoFold(room, folderUserId) {
        if (!room.gameRound)
            return;
        clearTurnTimer(room);
        ...
        socket.on('send_call', ({ roomId, userId, callType }) => {
            try {
                ...
                if (callType === 'FLOR') {
                    ...
                    if (currentTrick > 0 || callerCardsPlayed > 0)
                        return socket.emit('error_action', { message: 'Ya jugaste carta, no podés cantar Flor.' });
