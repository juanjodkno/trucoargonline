...
exports.getAllUsersListFresh = getAllUsersListFresh;
exports.requestDeposit = requestDeposit;
exports.requestDepositPersistent = requestDepositPersistent;
exports.getPendingDeposits = getPendingDeposits;
exports.getPendingDepositsFresh = getPendingDepositsFresh;
exports.recordTransaction = recordTransaction;
exports.adjustUserChipsAndRecord = adjustUserChipsAndRecord;
exports.approveDeposit = approveDeposit;
exports.rejectDeposit = rejectDeposit;
exports.getAllTransactions = getAllTransactions;
exports.getAllTransactionsFresh = getAllTransactionsFresh;
exports.getAdminMetrics = getAdminMetrics;
exports.getAdminMetricsFresh = getAdminMetricsFresh;
exports.resetRakeCounter = resetRakeCounter;
exports.deleteUser = deleteUser;
exports.getUserHistoryFresh = getUserHistoryFresh;
// src/auth/userService.ts
const pg_1 = require("pg");
const crypto_1 = __importDefault(require("crypto"));
exports.RAKE_PERCENTAGE = 7;
exports.RAKE_RATE = exports.RAKE_PERCENTAGE / 100;
function calculateMatchPayout(betPerPlayer) {
...
function syncUserCacheBalance(username, balance) {
    const clean = cleanUsername(username);
    const user = usersCache.find(u => cleanUsername(u.username) === clean);
    if (user)
        user.chips = Number(balance) || 0;
}
function settlementFromRow(r) {
    return {
        roomId: r.room_id,
...
function upsertSettlementCache(settlement) {
    const idx = settlementsCache.findIndex(s => s.roomId === settlement.roomId);
    if (idx >= 0)
        settlementsCache[idx] = settlement;
    else
...
async function registerUser(fullName, email, username, password) {
    ...
    if (usersCache.some(u => cleanUsername(u.username) === cleanUser))
    ...
    const newUser = {
        id: 'usr_' + crypto_1.default.randomBytes(4).toString('hex'), fullName: cleanFullName, email: cleanEmail,
...
async function loginUser(usernameOrEmail, password) {
    ...
    if (DATABASE_URL) {
        try {
            const res = await exports.pool.query('SELECT * FROM users WHERE LOWER(username) = $1 OR LOWER(email) = $1 LIMIT 1', [target]);
        ...
        user = usersCache.find(u => cleanUsername(u.username) === target ||
...
function resetUserPassword(username, newPass) {
    ...
    if (DATABASE_URL) {
        exports.pool.query('UPDATE users SET password_hash = $1, salt = $2 WHERE id = $3', [user.passwordHash, user.salt, user.id])
 ...
 */
async function getUserChipsFresh(username) {
    const clean = cleanUsername(username);
    if (!clean)
        return 0;
    if (!DATABASE_URL)
        return getUserChips(clean);
    try {
        const res = await exports.pool.query('SELECT chips FROM users WHERE LOWER(username) = $1 LIMIT 1', [clean]);
        if (!res.rows.length)
            return 0;
...
function modifyUserChips(username, amount) {
    ...
    const user = usersCache.find(u => cleanUsername(u.username) === clean);
    if (!user || user.chips + delta < 0)
        return false;
    user.chips += delta;
    if (DATABASE_URL) {
        exports.pool.query('UPDATE users SET chips = chips + $1 WHERE id = $2 AND chips + $1 >= 0 RETURNING chips', [delta, user.id]).then(res => {
            if (res.rows[0])
                syncUserCacheBalance(clean, Number(res.rows[0].chips));
        }).catch(err => console.error('Error actualizando fichas en BD:', err));
    }
    return true;
}
async function modifyUserChipsAtomic(username, amount) {
    const clean = cleanUsername(username);
    const delta = integerAmount(amount);
    if (!clean)
    ...
    try {
        const res = await exports.pool.query(`UPDATE users SET chips = chips + $1
       WHERE LOWER(username) = $2 AND chips + $1 >= 0
       RETURNING chips`, [delta, clean]);
        if (!res.rows.length)
...
async function applyRoomWalletOperation(idempotencyKey, roomId, username, operationType, amount) {
    const clean = cleanUsername(username);
    const delta = integerAmount(amount);
    if (!DATABASE_URL) {
        ...
        if (result.success)
            localWalletOperations.set(idempotencyKey, result);
        return result;
    }
    ...
    try {
       ...
       RETURNING idempotency_key`, [idempotencyKey, roomId, clean, operationType, delta]);
        if (!reserved.rows.length) {
            const previous = await client.query('SELECT balance_after FROM wallet_operations WHERE idempotency_key = $1', [idempotencyKey]);
       ...
       RETURNING chips`, [delta, clean]);
        ...
        await client.query('UPDATE wallet_operations SET balance_after = $1 WHERE idempotency_key = $2', [balance, idempotencyKey]);
        await client.query('COMMIT');
        syncUserCacheBalance(clean, balance);
        return { success: true, balance };
    }
...
async function debitRoomEntry(roomId, username, amount, role) {
    const bet = Math.max(0, integerAmount(amount));
    if (bet === 0)
        return { success: true, balance: await getUserChipsFresh(username) };
    return applyRoomWalletOperation(`${roomId}:ENTRY:${cleanUsername(username)}`, roomId, username, `ENTRY_${role}`, -bet);
}
/**
 ...
 */
async function refundRoomEntry(roomId, username, amount) {
    const clean = cleanUsername(username);
    const bet = Math.max(0, integerAmount(amount));
    if (bet === 0)
    ...
    if (!DATABASE_URL) {
        ...
        if (!entry?.success)
            return { success: false, message: 'No existe una entrada debitada para devolver.' };
    ...
    try {
       ...
       RETURNING idempotency_key`, [refundKey, roomId, clean, bet]);
        ...
        const changed = await client.query(`UPDATE users
       SET chips = chips + $1
...
async function settleMatchOnce(params) {
    ...
    if (!winner || !loser || winner === loser)
        return { success: false, message: 'Jugadores inválidos para liquidación.' };
    if (!DATABASE_URL) {
        ...
        const settlement = {
            ...
            winnerPrize: payout.winnerPrize, rakePercentage: exports.RAKE_PERCENTAGE,
    ...
    try {
        await client.query('BEGIN');
        const inserted = await client.query(`INSERT INTO match_settlements
       ...
       ON CONFLICT (room_id) DO NOTHING
       RETURNING created_at`, [params.roomId, winner, loser, payout.betPerPlayer, payout.grossPot,
            payout.winnerPrize, exports.RAKE_PERCENTAGE, payout.rakeAmount, params.finishReason]);
        if (!inserted.rows.length) {
            const existingRes = await client.query('SELECT * FROM match_settlements WHERE room_id = $1', [params.roomId]);
            await client.query('COMMIT');
            if (!existingRes.rows.length)
                return { success: false, message: 'No se pudo recuperar la liquidación existente.' };
        ...
        if (payout.betPerPlayer > 0) {
            const entries = await client.query(`SELECT LOWER(username) AS username, amount
         FROM wallet_operations
         WHERE room_id = $1
           AND operation_type IN ('ENTRY_CREATOR', 'ENTRY_GUEST')
           AND LOWER(username) = ANY($2::text[])`, [params.roomId, [winner, loser]]);
            const winnerEntry = entries.rows.find((r) => cleanUsername(r.username) === winner && Number(r.amount) === -payout.betPerPlayer);
            const loserEntry = entries.rows.find((r) => cleanUsername(r.username) === loser && Number(r.amount) === -payout.betPerPlayer);
        ...
        const locked = await client.query('SELECT username, chips FROM users WHERE LOWER(username) = ANY($1::text[]) ORDER BY LOWER(username) FOR UPDATE', [players]);
        if (locked.rows.length !== 2) {
            await client.query('ROLLBACK');
            return { success: false, message: 'No se encontraron ambos jugadores en la base de datos.' };
        }
        let winnerBalanceAfter = Number(locked.rows.find((r) => cleanUsername(r.username) === winner)?.chips) || 0;
        const loserBalanceAfter = Number(locked.rows.find((r) => cleanUsername(r.username) === loser)?.chips) || 0;
        if (payout.winnerPrize > 0) {
            const credit = await client.query('UPDATE users SET chips = chips + $1 WHERE LOWER(username) = $2 RETURNING chips', [payout.winnerPrize, winner]);
            if (!credit.rows.length) {
                await client.query('ROLLBACK');
        ...
        const finalRes = await client.query(`UPDATE match_settlements
       SET winner_balance_after = $1, loser_balance_after = $2
       WHERE room_id = $3
       RETURNING *`, [winnerBalanceAfter, loserBalanceAfter, params.roomId]);
        await client.query('COMMIT');
...
function updateUserAvatar(username, avatarId) {
    ...
    if (DATABASE_URL) {
        exports.pool.query('UPDATE users SET avatar = $1 WHERE id = $2', [avatarId, user.id])
...
async function getAllUsersListFresh() {
    if (!DATABASE_URL)
  ...
  `);
    ...
    for (const row of rows)
        syncUserCacheBalance(row.username, row.chips);
    return rows;
}
...
async function requestDepositPersistent(username, amount, reference) {
    const clean = cleanUsername(username);
    const value = Math.max(0, integerAmount(amount));
    if (!clean || value <= 0)
        return { success: false, message: 'Solicitud de depósito inválida.' };
    const newDep = {
        id: 'dep_' + crypto_1.default.randomBytes(8).toString('hex'),
        username: clean,
        amount: value,
        reference: reference || 'WhatsApp',
        status: 'PENDING',
    ...
    try {
        const userExists = await exports.pool.query('SELECT 1 FROM users WHERE LOWER(username) = $1 LIMIT 1', [clean]);
        if (!userExists.rows.length)
            return { success: false, message: 'Usuario no encontrado.' };
        await exports.pool.query(`INSERT INTO deposits (id, username, amount, reference, status, created_at)
       VALUES ($1, $2, $3, $4, 'PENDING', $5)`, [newDep.id, newDep.username, newDep.amount, newDep.reference, newDep.createdAt]);
...
async function getPendingDepositsFresh() {
    if (!DATABASE_URL)
...
function recordTransaction(type, username, amount, details = '') {
    // Compatibilidad local/legacy. Las operaciones reales usan funciones transaccionales.
    const tx = {
        id: 'tx_' + crypto_1.default.randomBytes(4).toString('hex'),
        type,
        username: cleanUsername(username),
        amount: integerAmount(amount),
        details,
    ...
    if (DATABASE_URL) {
        exports.pool.query('INSERT INTO transactions (id, type, username, amount, details, created_at) VALUES ($1, $2, $3, $4, $5, $6)', [tx.id, tx.type, tx.username, tx.amount, tx.details, tx.createdAt]).catch(err => console.error('Error guardando transacción en BD:', err));
    }
    return tx;
}
/**
 ...
 */
async function adjustUserChipsAndRecord(username, amount, type, details, idempotencyKey) {
    const clean = cleanUsername(username);
    const delta = integerAmount(amount);
    if (!clean || delta === 0)
    ...
    try {
        await client.query('BEGIN');
        if (idempotencyKey) {
            const existing = await client.query('SELECT id FROM transactions WHERE idempotency_key = $1 LIMIT 1', [idempotencyKey]);
            if (existing.rows.length) {
                const balance = await client.query('SELECT chips FROM users WHERE LOWER(username) = $1 LIMIT 1', [clean]);
                await client.query('COMMIT');
       ...
       RETURNING chips`, [delta, clean]);
        ...
        const txId = 'tx_' + crypto_1.default.randomBytes(8).toString('hex');
        await client.query(`INSERT INTO transactions
       (id, type, username, amount, details, created_at, idempotency_key)
       VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP, $6)`, [txId, type, clean, Math.abs(delta), details, idempotencyKey || null]);
        await client.query('COMMIT');
        const balance = Number(changed.rows[0].chips) || 0;
        syncUserCacheBalance(clean, balance);
        return { success: true, balance };
...
async function approveDeposit(depositId) {
    ...
    if (!DATABASE_URL) {
        ...
        if (!credit.success)
            return { success: false, message: credit.message || 'No se pudo acreditar.' };
        dep.status = 'APPROVED';
    ...
    try {
        await client.query('BEGIN');
        const locked = await client.query('SELECT * FROM deposits WHERE id = $1 FOR UPDATE', [depositId]);
        ...
        const changed = await client.query('UPDATE users SET chips = chips + $1 WHERE LOWER(username) = $2 RETURNING chips', [value, clean]);
        if (!changed.rows.length) {
            await client.query('ROLLBACK');
            return { success: false, message: 'Usuario no encontrado.' };
        }
       ...
       ON CONFLICT (idempotency_key) WHERE idempotency_key IS NOT NULL DO NOTHING`, [
            'tx_' + crypto_1.default.randomBytes(8).toString('hex'),
            clean,
            value,
...
async function rejectDeposit(depositId) {
    ...
    try {
        const res = await exports.pool.query(`UPDATE deposits
       SET status = 'REJECTED'
       WHERE id = $1 AND status = 'PENDING'
       RETURNING username`, [depositId]);
        if (!res.rows.length) {
            return { success: false, message: 'Solicitud no válida o ya procesada.' };
        }
        const depCache = depositsCache.find(d => d.id === depositId);
        if (depCache)
            depCache.status = 'REJECTED';
...
function teamSettlementDetails(s) {
    const loserTeam = s.winnerTeam === 'A' ? 'B' : 'A';
    const [winner1 = '', winner2 = ''] = s.winnerUsernames;
    const [loser1 = '', loser2 = ''] = s.loserUsernames;
    return `Mesa ${s.roomId}: Gana Equipo ${s.winnerTeam} (@${winner1}) saldo restante: $${Math.round(s.winnerBalancesAfter[winner1] || 0)} y (@${winner2}) saldo restante: $${Math.round(s.winnerBalancesAfter[winner2] || 0)} | Pierde Equipo ${loserTeam} (@${loser1}) saldo restante: $${Math.round(s.loserBalancesAfter[loser1] || 0)} y (@${loser2}) saldo restante: $${Math.round(s.loserBalancesAfter[loser2] || 0)} | Pozo: $${Math.round(s.grossPot)} | Rake ${s.rakePercentage}%: $${Math.round(s.rakeAmount)}`;
}
function teamSettlementAsTransaction(s) {
    return {
...
function settlementAsTransaction(s) {
    return {
        id: `match_${s.roomId}`,
...
function getAllTransactions(limit = 60) {
...
async function getAllTransactionsFresh(limit = 60) {
    const safeLimit = Math.max(1, Math.min(500, Math.floor(Number(limit) || 60)));
    if (!DATABASE_URL) {
        return getAllTransactions(safeLimit);
    }
    const [txRes, settlementRes, teamSettlementRes] = await Promise.all([
        exports.pool.query(`SELECT id, type, username, amount, details, created_at
       FROM transactions
       ORDER BY created_at DESC
       LIMIT $1`, [safeLimit]),
        exports.pool.query(`SELECT *
    ...
    const teamMatchTx = teamSettlementRes.rows.map(r => teamSettlementAsTransaction(teamSettlementFromRow(r)));
    return [
        ...normalTx,
        ...matchTx,
        ...teamMatchTx
...
function getAdminMetrics() {
    ...
    const afterRakeReset = (createdAt) => !localRakeCounterResetAt || new Date(createdAt).getTime() > localRakeCounterResetAt.getTime();
...
async function getAdminMetricsFresh() {
    if (!DATABASE_URL)
        return getAdminMetrics();
    const resetRes = await exports.pool.query(`
 ...
 */
async function resetRakeCounter() {
...
async function getUserHistoryFresh(username, limit = 100) {
    const clean = cleanUsername(username);
    ...
    */
    if (!DATABASE_URL) {
        ...
        const matchItems = settlementsCache
            .filter(s => cleanUsername(s.winnerUsername) === clean ||
    ...
    */
    for (const row of matchRes.rows) {
        const won = cleanUsername(row.winner_username) === clean;
        const bet = Number(row.bet_per_player) || 0;
        const prize = Number(row.winner_prize) || 0;
        items.push({
    ...
    */
    for (const row of teamRes.rows) {
        const winner1 = cleanUsername(row.winner1_username);
        const winner2 = cleanUsername(row.winner2_username);
        const won = winner1 === clean ||
            winner2 === clean;
        const bet = Number(row.bet_per_player) || 0;
        let prize = 0;
        if (winner1 === clean) {
