...
exports.TIME_ZONE = exports.LOSS_POINTS = exports.WIN_POINTS = void 0;
...
exports.WIN_POINTS = 3;
exports.LOSS_POINTS = -1;
exports.TIME_ZONE = 'America/Argentina/Buenos_Aires';
...
function rankResults(results, startsAt, endsAt, limit = 10) {
    const players = new Map();
    const seen = new Set();
    for (const result of results) {
        if (!(Number(result.betPerPlayer) > 0))
            continue;
        const time = new Date(result.createdAt).getTime();
        if (!(time >= Date.parse(startsAt) && time < Date.parse(endsAt)) ||
        ...
        for (const [name, won] of [
            ...
            if (won)
                ...
                player.losses++;
            player.points += won ? exports.WIN_POINTS : exports.LOSS_POINTS;
...
async function getWeeklyRanking(now = new Date(), username = null, includePrevious = true) {
    const bounds = weekBounds(now);
    let players;
    if (!process.env.DATABASE_URL) {
        players = rankResults((0, userService_1.getLocalRankingResults)(), bounds.startsAt, bounds.endsAt, Infinity);
    }
    else {
        const result = await userService_1.pool.query(`
      WITH results AS (
    ...
    const previous = includePrevious
        ? await getWeeklyRanking(new Date(Date.parse(bounds.startsAt) - 1), null, false)
        : null;
    return {
        personal,
        previous: previous ? {
        ...
        winPoints: exports.WIN_POINTS,
        lossPoints: exports.LOSS_POINTS,
        modes: 'Solo 1 vs. 1',
