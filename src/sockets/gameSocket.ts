...
interface ActiveRoom {
  ...
  trucoOwner: string | null;
   ...
   // Candados anti-duplicados del juego.
  // No modifican reglas ni puntuaciones: solo evitan procesar dos veces
  // la misma acción recibida por doble clic, lag o paquetes repetidos.
  roundEnding?: boolean;
  recentClientActions?: Map<string, number>;
  ...
  // Modo demostración contra la máquina. Nunca toca saldo ni liquidaciones.
  isBotGame?: boolean;
  botId?: string;
  botThinkingTimeout?: NodeJS.Timeout;

  // Historial visible de la partida, guardado en la sala del servidor para reconexiones.
  logs: string[];
...
export function setupSocketEvents(io: Server) {
  ...
  function getAuthenticatedUserId(room: ActiveRoom, socketId: string): string | null {
    ...
    function isDuplicateClientAction(
    room: ActiveRoom,
    userId: string,
    action: string,
    windowMs: number = 700
    ...
    if (!room.recentClientActions) {
      room.recentClientActions = new Map<string, number>();
    }
    ...
    if (previous !== undefined && (now - previous) < windowMs) {
      console.warn(
        `[DUPLICATE ACTION BLOCKED] room=${room.roomId} user=${userId} action=${action}`
      );
      return true;
    }
  ...
  function releaseRoomPresence(room: ActiveRoom) {
    if (room.isBotGame) return;
    releaseActiveGame(room.creatorId, '1v1', room.roomId);
    if (room.guestId) releaseActiveGame(room.guestId, '1v1', room.roomId);
  }


  function isBotPlayer(room: ActiveRoom, userId?: string | null): boolean {
    return !!room.isBotGame && !!room.botId && !!userId && room.botId.toLowerCase() === userId.toLowerCase();
  }

  function getAllCardsForUser(room: ActiveRoom, userId: string): Card[] {
    if (!room.gameRound) return [];
    const hand = userId.toLowerCase() === room.creatorId.toLowerCase() ? room.gameRound.p1 : room.gameRound.p2;
    return hand.cards.concat(hand.cardsPlayed.filter(Boolean) as Card[]);
  }

  function getBotHand(room: ActiveRoom) {
    if (!room.gameRound || !room.botId) return null;
    return room.botId.toLowerCase() === room.creatorId.toLowerCase() ? room.gameRound.p1 : room.gameRound.p2;
  }

  function getHumanId(room: ActiveRoom): string {
    if (!room.botId) return room.creatorId;
    return room.creatorId.toLowerCase() === room.botId.toLowerCase() ? (room.guestId || room.creatorId) : room.creatorId;
  }

  function getBotPower(room: ActiveRoom): number {
  ...
  function isBotActionPending(room: ActiveRoom): boolean {
    if (!room.isBotGame || !room.botId || !room.gameRound || room.disconnectedUser) return false;
  ...
  function scheduleBotAction(room: ActiveRoom) {
    if (!isBotActionPending(room)) return;
    if (room.botThinkingTimeout) clearTimeout(room.botThinkingTimeout);
  ...
  function emitBotCall(room: ActiveRoom, callType: string, category: 'ENVIDO' | 'FLOR' | 'TRUCO', awaitingResponseFrom: string, extra: Record<string, unknown> = {}) {
  ...
  function botRaiseEnvido(room: ActiveRoom, callType: string) {
    if (!room.gameRound || !room.botId) return;
  ...
  function botRespondToEnvido(room: ActiveRoom) {
    if (!room.gameRound || !room.botId) return;
    const score = getEnvidoDetails(getAllCardsForUser(room, room.botId)).score;
    const lastCall = room.envidoChain[room.envidoChain.length - 1] || 'ENVIDO';
    const roll = Math.random();

    if (lastCall === 'ENVIDO' && score >= 31 && roll < 0.32) return botRaiseEnvido(room, 'REAL_ENVIDO');
    if (lastCall === 'ENVIDO' && score >= 29 && roll < 0.52) return botRaiseEnvido(room, 'ENVIDO_ENVIDO');
    if (lastCall === 'ENVIDO_ENVIDO' && score >= 31 && roll < 0.40) return botRaiseEnvido(room, 'REAL_ENVIDO');
    if (lastCall === 'REAL_ENVIDO' && score >= 32 && roll < 0.22) return botRaiseEnvido(room, 'FALTA_ENVIDO');
  ...
  function botRaiseFlor(room: ActiveRoom, callType: 'CONTRAFLOR' | 'CONTRAFLOR_AL_JUEGO') {
    if (!room.gameRound || !room.botId) return;
    const humanId = getHumanId(room);
  ...
  function botRespondToFlor(room: ActiveRoom) {
    if (!room.gameRound || !room.botId) return;
    const cards = getAllCardsForUser(room, room.botId);
    if (!hasFlor(cards)) return resolveFlorDeclined(room, room.botId);

    const florScore = calculateFlor(cards);
  ...
  function botRespondToTruco(room: ActiveRoom) {
    if (!room.gameRound || !room.botId) return;
    const humanId = getHumanId(room);
    ...
    if (stake === 2 && room.pendingTrucoAfterEnvido && !room.gameRound.envidoResolved) {
      const envidoScore = getEnvidoDetails(getAllCardsForUser(room, room.botId)).score;
      const botHasFlor = room.withFlor && hasFlor(getAllCardsForUser(room, room.botId));
      if (botHasFlor) return botInitiateFlor(room);
      if (envidoScore >= 28 && Math.random() < 0.55) return botRaiseEnvido(room, envidoScore >= 32 ? 'REAL_ENVIDO' : 'ENVIDO');
    }
  ...
  function botInitiateFlor(room: ActiveRoom): boolean {
    if (!room.gameRound || !room.botId || !room.withFlor) return false;
    const botCards = getAllCardsForUser(room, room.botId);
    if (!hasFlor(botCards)) return false;

    const humanId = getHumanId(room);
    const humanCards = getAllCardsForUser(room, humanId);
    const humanHasFlor = hasFlor(humanCards);
  ...
  function botInitiateEnvido(room: ActiveRoom): boolean {
    if (!room.gameRound || !room.botId) return false;
    if (room.gameRound.currentTrickIndex !== 0 || room.gameRound.envidoResolved || room.gameRound.florResolved) return false;
    const hand = getBotHand(room);
    if (!hand || hand.cardsPlayed.filter(Boolean).length > 0) return false;
    if (room.envidoChain.length > 0 || room.florChain.length > 0) return false;
    if (room.withFlor && hasFlor(getAllCardsForUser(room, room.botId))) return false;

    const score = getEnvidoDetails(getAllCardsForUser(room, room.botId)).score;
    if (score < 28 && Math.random() > 0.08) return false;
    const callType = score >= 32 && Math.random() < 0.30 ? 'REAL_ENVIDO' : 'ENVIDO';
    botRaiseEnvido(room, callType);
    return true;
  }

  function botInitiateTruco(room: ActiveRoom): boolean {
    if (!room.gameRound || !room.botId || room.gameRound.awaitingResponseFrom) return false;
    const humanId = getHumanId(room);
    const power = getBotPower(room);

    if (room.trucoLevel === 1) {
      if (power < 0.55 && Math.random() > 0.10) return false;
  ...
  function chooseBotCard(room: ActiveRoom): Card | null {
    if (!room.gameRound || !room.botId) return null;
    const hand = getBotHand(room);
    ...
    if (rivalPlayed) {
      ...
      if (winning.length > 0) return winning[0]; // gana gastando la carta menos fuerte posible
      return weakestFirst[0];
    }

    // Al salir, conserva las mejores cartas salvo cuando el partido o el canto está muy jugado.
    if (hand.cards.length === 1) return hand.cards[0];
  ...
  function runBotAction(room: ActiveRoom) {
    if (!room.isBotGame || !room.botId || !room.gameRound || room.disconnectedUser) return;
    ...
    if (room.isDeclaringEnvido) {
      if (isBotPlayer(room, room.envidoDeclarer)) {
        ...
        const score = room.isFlorDeclaration ? calculateFlor(cards) : getEnvidoDetails(cards).score;
        if (room.highestEnvidoScore === 0 || score > room.highestEnvidoScore) executeDeclareEnvido(room, botId, score);
        else executeSonBuenas(room, botId);
      }
      return;
    }
    ...
    if (room.gameRound.awaitingResponseFrom) {
      if (isBotPlayer(room, room.gameRound.awaitingResponseFrom)) {
        if (room.envidoPendingCaller) return botRespondToEnvido(room);
        if (room.florPendingCaller) return botRespondToFlor(room);
        return botRespondToTruco(room);
      }
      return;
    }

    if (!isBotPlayer(room, room.gameRound.currentTurn)) return;
    ...
    if (canStillCallTantos && !room.gameRound.envidoResolved && !room.gameRound.florResolved) {
      if (room.withFlor && hasFlor(getAllCardsForUser(room, botId)) && botInitiateFlor(room)) return;
      if (botInitiateEnvido(room)) return;
    }

    if (botInitiateTruco(room)) return;

    const card = chooseBotCard(room);
    if (card) executePlayCard(room, botId, card.id);
  }
  ...
  function startEnvidoDeclarationPhase(room: ActiveRoom, isFlor: boolean = false, acceptedBy?: string) {
    clearTurnTimer(room);
  ...
  function resolveTrucoFold(
  room: ActiveRoom,
  folderUserId: string,
  reason: string = 'NO_QUIERO_TRUCO'
) {
  ...
  if (room.roundEnding) {
    console.warn(
      `[ROUND ALREADY ENDING] ${room.roomId}: acción duplicada ignorada (${reason})`
    );
    return;
  }
  ...
  clearTurnTimer(room);
    ...
    const foldedTrucoCallerDuringPendingEnvido = !!(
      reason === 'ME_VOY_AL_MAZO' &&
      pendingTrucoAtFold &&
    ...
    if (room.florChain.length > 0 && !room.gameRound.florResolved) {
      ...
      const pendingUnansweredEnvidoAtFold = !!(
        reason === 'ME_VOY_AL_MAZO' &&
  ...
  function sendFullSync(socket: Socket, room: ActiveRoom, userId: string) {
    ...
    socket.emit('sync_game_state', {
      ...
      rivalAvatar: room.isBotGame ? 'gaucho' : (rivalUsername ? getUserAvatar(rivalUsername) : 'gaucho')
    });
  }
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('game_log_line', ({ roomId, message }) => {
      ...
      if (!room) return;
      if (!getAuthenticatedUserId(room, socket.id)) return;

      const line = String(message ?? '').trim().slice(0, 1200);
      if (!line) return;
      if (room.logs[room.logs.length - 1] === line) return;
      room.logs.push(line);
      if (room.logs.length > 500) room.logs.splice(0, room.logs.length - 500);
    });
  ...
  */
    ...
    try {
      ...
      if (!(await userExistsFresh(cleanUser))) {
        ...
        socket.emit(
          ...
          {
            message:
              'Tu cuenta ya no existe. Volvé a iniciar sesión.'
          }
        );
      ...
      return socket.emit(
        ...
        {
          message:
            'No se pudo validar tu cuenta. Intentá nuevamente.'
        }
      );
  ...
  const canReconnect =
    !!room && (
      room.creatorId.toLowerCase() === cleanUser ||
      (
        !room.isBotGame &&
        !!room.guestId &&
  ...
  if (room && canReconnect) {
    ...
    socket.on('start_bot_game', ({ userId }) => {
      ...
      if (!cleanUser) return socket.emit('error_action', { message: 'No se pudo iniciar el modo de prueba.' });
      ...
      const roomId = 'prueba_' + crypto.randomBytes(3).toString('hex');
      ...
      socket.emit('game_ready', {
        roomId,
        creatorId: cleanUser,
        creatorAvatar: getUserAvatar(cleanUser),
        guestId: botId,
        guestAvatar: 'gaucho',
        pot: 0,
        targetPoints: 15,
      ...
      setTimeout(() => {
        if (rooms.get(roomId) === room) dealAutoHand(room);
      }, 650);
    });
    ...
    socket.on('create_room', async ({ userId, betAmount, targetPoints, withFlor }) => {
      ...
      let claimedRoomId: string | null = null;
      ...
      try {
        ...
        if (!claimActiveGame(cleanUser, '1v1', roomId)) {
        ...
        if (!debit.success) {
          releaseActiveGame(cleanUser, '1v1', roomId);
          claimedRoomId = null;
        ...
        if (debitSucceeded && claimedRoomId && !roomStored) {
          try { await refundRoomEntry(claimedRoomId, cleanUser, Number(betAmount) || 0); } catch {}
        }
        if (claimedRoomId && !roomStored) releaseActiveGame(cleanUser, '1v1', claimedRoomId);
      ...
      try {
        ...
        for (const [pendingRoomId, pendingRoom] of rooms.entries()) {
          ...
          if (pendingRoom.creatorId.toLowerCase() === cleanUser && !pendingRoom.guestId) {
            ...
            releaseActiveGame(cleanUser, '1v1', pendingRoomId);
          }
        }

        if (!claimActiveGame(cleanUser, '1v1', room.roomId)) {
          room.joiningUser = null;
          return socket.emit('error_action', { message: 'Ya estás participando en otra mesa.' });
        }
        claimedTarget = true;
        ...
        if (!debit.success) {
          room.joiningUser = null;
          releaseActiveGame(cleanUser, '1v1', room.roomId);
          claimedTarget = false;
          return socket.emit('error_action', { message: debit.message || 'Saldo insuficiente.' });
        }
        debitSucceeded = true;
        ...
        if (debitSucceeded && !seated) {
          try { await refundRoomEntry(String(roomId || ''), cleanUser, debitBet); } catch {}
        }
        if (claimedTarget && !seated) releaseActiveGame(cleanUser, '1v1', String(roomId || ''));
        ...
        if (!room.guestId && room.creatorSocketId === socket.id) {
          ...
          room.waitingTimeout = setTimeout(async () => {
            ...
            if (activeRoom && !activeRoom.guestId) {
              ...
              rooms.delete(roomId);
              releaseActiveGame(activeRoom.creatorId, '1v1', activeRoom.roomId);
              broadcastTables();
            }
          }, 30 * 60 * 1000);
    ...
    socket.on('declare_envido_points', ({ roomId }) => {
      ...
      const actualPoints = room.isFlorDeclaration
        ? calculateFlor(allCards)
        : getEnvidoDetails(allCards).score;
    ...
    socket.on('send_call', ({ roomId, callType }) => {
      try {
        ...
        if (
          isDuplicateClientAction(
            room,
            authUser,
            `send_call:${String(callType || '')}`,
            700
          )
        ) {
          return;
